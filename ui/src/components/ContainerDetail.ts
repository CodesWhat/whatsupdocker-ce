...
export default defineComponent({
  ...
  computed: {
    ...
    hookTimeout(): number {
      return parseInt(this.container.labels?.['dd.hook.timeout'] ?? this.container.labels?.['wud.hook.timeout'] ?? '60000', 10);
    },
    hasHooks(): boolean {
    ...
    rollbackWindow(): number {
      return parseInt(this.container.labels?.['dd.rollback.window'] ?? this.container.labels?.['wud.rollback.window'] ?? '300000', 10);
    },
    rollbackInterval(): number {
      return parseInt(this.container.labels?.['dd.rollback.interval'] ?? this.container.labels?.['wud.rollback.interval'] ?? '10000', 10);
    },
  },
